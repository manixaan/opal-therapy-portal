...
describe('scalar model', () => {
  ...
  test('an Opal-issued value is badged as issued, and is still editable', () => {
    // Opal supplies the reference so a therapist is never told their own
    // letter's id is "Missing" — but a practice that numbers its own
    // correspondence is not overruled by that convenience.
    ...
    expect(docId.missing).toBe(false);
    expect(docId.editable).toBe(true);
...
describe('missing summary', () => {
  ...
  test('an excluded tag is never listed — it is an answered question', () => {
    const s = ltrMissingSummary(
      ['OPAL_THERAPIST_AHPRA_REGISTRATION'], manifest().scalarData,
      ['OPAL_THERAPIST_AHPRA_REGISTRATION', 'OPAL_CLIENT_PREFERRED_NAME']
    );
    expect(s.count).toBe(0);
...
describe('exclusion — a third state, not a flavour of missing', () => {
  const fieldsOf = (m) => ltrScalarModel(m).reduce((a, g) => a.concat(g.fields), []);
  const one = (m, tag) => fieldsOf(m).find((f) => f.tag === tag);

  test('the excluded set comes from the manifest and nowhere else', () => {
    expect(ltrExcludedSet({ excludedTags: ['A'] })).toEqual({ A: true });
    expect(ltrExcludedSet({})).toEqual({});
    expect(ltrExcludedSet(null)).toEqual({});
    // A draft-level field is NOT consulted: the manifest is the only authority
    // on what the letter will actually contain.
    expect(ltrExcludedSet({ excludedFields: ['A'] })).toEqual({});
  });

  test('an excluded field is excluded, not missing, carries no value, is not editable', () => {
    const m = manifest({ excludedTags: ['OPAL_THERAPIST_ROLE'] });
    const f = one(m, 'OPAL_THERAPIST_ROLE');
  ...
  test('an excluded field that WAS missing stops counting as missing', () => {
    const f = one(manifest({ excludedTags: ['OPAL_CLIENT_PREFERRED_NAME'] }), 'OPAL_CLIENT_PREFERRED_NAME');
    expect(f.excluded).toBe(true);
    expect(f.missing).toBe(false);
  });

  test('the Opal-issued document id can be excluded too', () => {
    const f = one(manifest({ excludedTags: ['OPAL_LETTER_DOCUMENT_ID'] }), 'OPAL_LETTER_DOCUMENT_ID');
    expect(f.excluded).toBe(true);
    expect(f.value).toBeNull();
  });

  test('un-excluding restores the resolved value with no extra machinery', () => {
    expect(one(manifest(), 'OPAL_THERAPIST_ROLE').value).toBe('Occupational Therapist');
    expect(one(manifest(), 'OPAL_THERAPIST_ROLE').excluded).toBe(false);
  });

  test('toggling returns a new list, and un-toggling removes exactly one tag', () => {
    expect(ltrToggleExcluded([], 'A', true)).toEqual(['A']);
    expect(ltrToggleExcluded(['A'], 'B', true)).toEqual(['A', 'B']);
    expect(ltrToggleExcluded(['A', 'B'], 'A', false)).toEqual(['B']);
    expect(ltrToggleExcluded(['A'], 'A', true)).toEqual(['A']);
    expect(ltrToggleExcluded(null, 'A', false)).toEqual([]);
  });

  test('an excluded value NEVER blocks generation, required or not', () => {
    const blocked = ltrBlockingIssues(draft({
      letterDetails: { letterDate: '2026-08-10', subject: '', reportingPeriod: null },
    }), { scalarTags: [] });
    expect(blocked.map((i) => i.key)).toContain('subject');

    // The SERVER restates the exclusion in the wizard's own field vocabulary,
    // so the front end needs no merge tag to honour it.
    const allowed = ltrBlockingIssues(draft({
      letterDetails: { letterDate: '2026-08-10', subject: '', reportingPeriod: null },
      manifest: manifest({ excludedTags: ['OPAL_LETTER_SUBJECT'] }),
      excludedLetterFields: { subject: true },
    }), { scalarTags: [] });
    expect(allowed.map((i) => i.key)).not.toContain('subject');
  });

  test('the field-name restatement comes from the server, never invented here', () => {
    expect(ltrExcludedFields({ excludedLetterFields: { subject: true } })).toEqual({ subject: true });
    expect(ltrExcludedFields({})).toEqual({});
    expect(ltrExcludedFields(null)).toEqual({});
    // A list is not the object shape the contract promises; it is refused
    // rather than half-read.
    expect(ltrExcludedFields({ excludedLetterFields: ['subject'] })).toEqual({});
  });

  test('a template-required scalar that is excluded stops blocking too', () => {
    const template = { scalarTags: [{ tag: 'OPAL_LETTER_SALUTATION', required: true }] };
    const d = draft({ missingFields: ['OPAL_LETTER_SALUTATION'] });
    expect(ltrBlockingIssues(d, template).map((i) => i.key)).toContain('OPAL_LETTER_SALUTATION');

    const excluded = draft({
      missingFields: ['OPAL_LETTER_SALUTATION'],
      manifest: manifest({ excludedTags: ['OPAL_LETTER_SALUTATION'] }),
    });
    expect(ltrBlockingIssues(excluded, template).map((i) => i.key)).not.toContain('OPAL_LETTER_SALUTATION');
  });

  test('the preview omits what the letter will omit', () => {
    const m = ltrPreviewModel(draft({
      manifest: manifest({
        excludedTags: [
          'OPAL_ORGANISATION_EMAIL',
          'OPAL_LETTER_DOCUMENT_ID',
          'OPAL_THERAPIST_ROLE',
        ],
      }),
      excludedLetterFields: { documentId: true },
    }));
    // An excluded value is not shown as a value, and not shown as missing.
    expect(m.letterhead.map((f) => f.tag)).not.toContain('OPAL_ORGANISATION_EMAIL');
    expect(m.signatureFields.map((f) => f.tag)).not.toContain('OPAL_THERAPIST_ROLE');
    // The footer reference goes with it, even though it is drawn from the
    // draft's own letterDetails snapshot rather than the manifest's scalars.
    expect(m.documentId).toBe('');
    expect(m.excludedFields.map((f) => f.tag).sort()).toEqual([
      'OPAL_LETTER_DOCUMENT_ID', 'OPAL_ORGANISATION_EMAIL', 'OPAL_THERAPIST_ROLE',
    ]);
  });

  test('excluding the CC removes the CC line from the preview', () => {
    const m = ltrPreviewModel(draft({
      manifest: manifest({ excludedTags: ['OPAL_LETTER_CC'] }),
      excludedLetterFields: { cc: true },
    }));
    expect(m.cc.willAppear).toBe(false);
    expect(m.cc.text).toContain('No CC line will appear');
  });

  test('excluding an addressing field blanks it in the preview', () => {
    const m = ltrPreviewModel(draft({
      manifest: manifest({ excludedTags: ['OPAL_LETTER_RECIPIENT_ROLE', 'OPAL_LETTER_SUBJECT'] }),
      excludedLetterFields: { recipientRole: true, subject: true },
    }));
    expect(m.recipient.role).toBe('');
    expect(m.recipient.name).toBe('Priya Nand'); // untouched
    expect(m.subject).toBe('');
  });
});

describe('the blank-or-exclude note', () => {
  test('is worded exactly as agreed, and says both real options', () => {
    expect(LTR_BLANK_OR_EXCLUDE_NOTE).toBe(
  ...
  test('it is worded IDENTICALLY in the FCA report builder', () => {
    // The same promise about the same thing. Two wordings would read as two
    // different rules.
    /* eslint-disable-next-line global-require */
    const fca = require('../../frontend/current/fca.js');
    expect(LTR_BLANK_OR_EXCLUDE_NOTE).toBe(fca.FCA_BLANK_OR_EXCLUDE_NOTE);
  });
});

// ══ Source guards ══════════════════════════════════════════════════════════
// The repository's shell-parsing convention (frontend-stage3-guards,
// whodas-frontend-guards): the shipped asset is read and asserted on
// structurally, because there is no DOM harness.

describe('letter.js source guards', () => {
  const SRC = fs.readFileSync(
    path.join(__dirname, '..', '..', 'frontend', 'current', 'letter.js'), 'utf8'
  );

  test('the note is rendered, not merely defined', () => {
    expect(SRC).toContain('LTR_BLANK_OR_EXCLUDE_NOTE');
    expect(SRC).toMatch(/ltrEsc\(LTR_BLANK_OR_EXCLUDE_NOTE\)/);
    // A quiet inline note, not an alarming callout.
    expect(SRC).toContain('class="ltr-inline-note"');
    expect(SRC).not.toMatch(/ltr-note-warn[^>]*>[^<]*If we do not hold/);
  });

  test('every merged-value row carries a real, labelled Exclude control', () => {
    expect(SRC).toContain('data-ltr-check="exclude"');
    expect(SRC).toContain('>Exclude<');
    expect(SRC).toMatch(/<label for="' \+ ltrEsc\(exId\)/);
    expect(SRC).toContain("type=\"checkbox\" id=\"' + ltrEsc(exId)");
  });

  test('an excluded row says what will happen, and disables its input', () => {
    expect(SRC).toContain('Excluded — nothing will be inserted');
    expect(SRC).toContain('ltr-field-excluded');
    expect(SRC).toMatch(/f\.editable \? '' : ' disabled'/);
  });

  test('no value is ever fabricated for a recipient or a reviewer', () => {
    // The recipient block is typed by the therapist or chosen from a saved
    // contact; nothing here supplies a default name, role or address.
    expect(SRC).not.toMatch(/recipient\s*[:=]\s*\{[^}]*name\s*:\s*['"][A-Za-z]/);
    expect(SRC).not.toMatch(/salutation\s*[:=]\s*['"](Dear|To whom)/i);
    expect(SRC).not.toMatch(/reviewer\s*[:=]\s*['"]/i);
  });

  test('the exclusion state is read from the manifest, never computed locally', () => {
    expect(SRC).toMatch(/function ltrExcludedSet\(manifest\)/);
    expect(SRC).toMatch(/m\.excludedTags\s*\)\s*\?\s*m\.excludedTags\s*:\s*\[\]/);
  });
});
