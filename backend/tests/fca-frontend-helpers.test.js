...
describe('source badge labels — the exact strings, and nothing else', () => {
  test('the mapping is exactly the contract labels', () => {
    expect(FCA_SOURCE_LABELS).toEqual({
      ...
      // and status. Calling those "Missing" was never a fact.
      server: 'Generated by Opal',
      missing: 'Missing',
    });
    expect(FCA_SOURCE_ORDER)
      .toEqual(['splose', 'client_profile', 'report_override', 'server', 'missing']);
  });
  ...
  test('each layer maps to its own label', () => {
    ...
    expect(fcaSourceLabel('server')).toBe('Generated by Opal');
...
describe('exclusion — a third state, not a flavour of missing', () => {
  const base = {
    scalarData: {
      OPAL_REPORT_REVIEWER_NAME: null,
      OPAL_REPORT_REVIEWER_ROLE: 'Senior OT',
      OPAL_REPORT_DOCUMENT_ID: 'FCA-1A2B3C4D',
    },
    scalarSources: {
      OPAL_REPORT_REVIEWER_NAME: 'missing',
      OPAL_REPORT_REVIEWER_ROLE: 'report_override',
      OPAL_REPORT_DOCUMENT_ID: 'server',
    },
  };
  const fieldsOf = (m) => fcaScalarModel(m, []).reduce((a, g) => a.concat(g.fields), []);
  const one = (m, tag) => fieldsOf(m).find((f) => f.tag === tag);

  test('the excluded set comes from the manifest and nowhere else', () => {
    expect(fcaExcludedSet({ excludedTags: ['A', 'B'] })).toEqual({ A: true, B: true });
    expect(fcaExcludedSet({})).toEqual({});
    expect(fcaExcludedSet(null)).toEqual({});
    // A draft-level field is NOT consulted: the manifest is the only authority
    // on what the document will actually contain.
    expect(fcaExcludedSet({ excludedFields: ['A'] })).toEqual({});
  });

  test('an excluded field is excluded, not missing — and carries no value', () => {
    const m = { ...base, excludedTags: ['OPAL_REPORT_REVIEWER_ROLE'] };
    const f = one(m, 'OPAL_REPORT_REVIEWER_ROLE');
  ...
  test('an excluded field that WAS missing stops counting as missing', () => {
    const m = { ...base, excludedTags: ['OPAL_REPORT_REVIEWER_NAME'] };
    const f = one(m, 'OPAL_REPORT_REVIEWER_NAME');
    expect(f.excluded).toBe(true);
    expect(f.missing).toBe(false);
  });

  test('an Opal-issued field can be excluded too', () => {
    const m = { ...base, excludedTags: ['OPAL_REPORT_DOCUMENT_ID'] };
    const f = one(m, 'OPAL_REPORT_DOCUMENT_ID');
    expect(f.excluded).toBe(true);
    expect(f.value).toBeNull();
  });

  test('un-excluding restores the resolved value with no extra machinery', () => {
    expect(one(base, 'OPAL_REPORT_REVIEWER_ROLE').value).toBe('Senior OT');
    expect(one(base, 'OPAL_REPORT_REVIEWER_ROLE').excluded).toBe(false);
  });

  test('an excluded field is never offered for saving to the client profile', () => {
    const m = {
      scalarData: { OPAL_CLIENT_PRONOUNS: 'they/them' },
      scalarSources: { OPAL_CLIENT_PRONOUNS: 'report_override' },
      excludedTags: ['OPAL_CLIENT_PRONOUNS'],
    };
    expect(fcaProfileSavableFields(m, ['OPAL_CLIENT_PRONOUNS'])).toEqual([]);
    delete m.excludedTags;
    expect(fcaProfileSavableFields(m, ['OPAL_CLIENT_PRONOUNS'])).toHaveLength(1);
  });

  test('the missing summary never lists an excluded field', () => {
    const missing = ['OPAL_REPORT_REVIEWER_NAME', 'OPAL_REPORT_AUTHORISED_RECIPIENTS'];
    const all = fcaMissingSummary(missing, {}, []);
    expect(all.count).toBe(2);

    const some = fcaMissingSummary(missing, {}, ['OPAL_REPORT_REVIEWER_NAME']);
    expect(some.count).toBe(1);
    expect(some.tags).toEqual(['OPAL_REPORT_AUTHORISED_RECIPIENTS']);

    // A blank in scalarData is caught the same way, and excluded the same way.
    expect(fcaMissingSummary([], { OPAL_CLIENT_EMAIL: '' }, ['OPAL_CLIENT_EMAIL']).count).toBe(0);
  });

  test('toggling returns a new list, and un-toggling removes exactly one tag', () => {
    expect(fcaToggleExcluded([], 'A', true)).toEqual(['A']);
    expect(fcaToggleExcluded(['A'], 'B', true)).toEqual(['A', 'B']);
    expect(fcaToggleExcluded(['A', 'B'], 'A', false)).toEqual(['B']);
    // Idempotent: excluding twice does not duplicate.
    expect(fcaToggleExcluded(['A'], 'A', true)).toEqual(['A']);
    expect(fcaToggleExcluded(null, 'A', false)).toEqual([]);
    const src = ['A'];
    expect(fcaToggleExcluded(src, 'B', true)).not.toBe(src);
    expect(src).toEqual(['A']);
  });

  test('the preview reports exclusions and the values Opal issued', () => {
    const m = fcaPreviewModel({
      manifest: {
        ...base,
        excludedTags: ['OPAL_REPORT_REVIEWER_ROLE'],
        sections: [{ tag: 'A', kind: 'required', title: 'A', included: true, order: 0 }],
      },
    });
    expect(m.excludedFields.map((f) => f.tag)).toEqual(['OPAL_REPORT_REVIEWER_ROLE']);
    // The document-control line is built from the server's own attribution,
    // never from a hard-coded tag list.
    expect(m.documentControl.map((f) => f.tag)).toEqual(['OPAL_REPORT_DOCUMENT_ID']);
    expect(m.documentControl[0].value).toBe('FCA-1A2B3C4D');
  });
});

describe('the blank-or-exclude note', () => {
  test('is worded exactly as agreed, and says both real options', () => {
    expect(FCA_BLANK_OR_EXCLUDE_NOTE).toBe(
...
// structurally, because there is no DOM harness. These pin the properties it
// would be clinically misleading to lose in a refactor.

describe('fca.js source guards', () => {
  const SRC = fs.readFileSync(
    path.join(__dirname, '..', '..', 'frontend', 'current', 'fca.js'), 'utf8'
  );

  test('the note is rendered, not merely defined', () => {
    expect(SRC).toContain('FCA_BLANK_OR_EXCLUDE_NOTE');
    expect(SRC).toMatch(/fcaEsc\(FCA_BLANK_OR_EXCLUDE_NOTE\)/);
    // A quiet inline note, not an alarming callout.
    expect(SRC).toContain('class="fca-inline-note"');
    expect(SRC).not.toMatch(/fca-note-warn[^>]*>[^<]*If we do not hold/);
  });

  test('every scalar row carries a real, labelled Exclude control', () => {
    expect(SRC).toContain('data-fca-check="exclude"');
    expect(SRC).toContain('>Exclude<');
    // A real <label for>, not a bare checkbox with placeholder text.
    expect(SRC).toMatch(/<label for="' \+ fcaEsc\(exId\)/);
    expect(SRC).toContain("type=\"checkbox\" id=\"' + fcaEsc(exId)");
  });

  test('an excluded row says what will happen, and disables its input', () => {
    expect(SRC).toContain('Excluded — nothing will be inserted');
    expect(SRC).toContain('fca-field-excluded');
    expect(SRC).toMatch(/f\.excluded \? ' disabled' : ''/);
  });

  test('no value is ever fabricated for a reviewer or a recipient', () => {
    // The four report-control tags Opal issues are named in the SERVER's
    // template map, never here — and no default, fallback or literal for a
    // reviewer or a recipient list exists anywhere in this file.
    expect(SRC).not.toContain('OPAL_REPORT_REVIEWER_NAME');
    expect(SRC).not.toContain('OPAL_REPORT_REVIEWER_ROLE');
    expect(SRC).not.toContain('OPAL_REPORT_AUTHORISED_RECIPIENTS');
    expect(SRC).not.toContain('OPAL_REPORT_ISSUE_DATE');
    expect(SRC).not.toMatch(/reviewer\s*[:=]\s*['"]/i);
    expect(SRC).not.toMatch(/recipients?\s*[:=]\s*['"][A-Za-z]/i);
  });

  test('the exclusion state is read from the manifest, never computed locally', () => {
    // fcaExcludedSet is the ONLY reader, and it only ever looks at
    // manifest.excludedTags.
    expect(SRC.match(/excludedTags/g).length).toBeGreaterThan(0);
    expect(SRC).toMatch(/function fcaExcludedSet\(manifest\)/);
