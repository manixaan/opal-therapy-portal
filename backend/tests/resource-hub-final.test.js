...
describe('file surfaces agree with the detail surface', () => {
  test('both file routes apply the same visibleTo rule as the detail route', () => {
    const dl = ROUTES.slice(ROUTES.indexOf("router.get('/api/rh2/files/:fileId'"),
      ROUTES.indexOf("router.get('/api/rh2/resources/:id/files'"));
    ...
    expect(dl).toMatch(/visibleTo\(req\.user/);
    expect(meta).toMatch(/visibleTo\(req\.user, resource\)/);
  });

  test('the download query selects the parent status the rule needs', () => {
    const dl = ROUTES.slice(ROUTES.indexOf("router.get('/api/rh2/files/:fileId'"),
      ROUTES.indexOf("router.get('/api/rh2/resources/:id/files'"));
    expect(dl).toMatch(/r\.status AS resource_status/);
  });
});
