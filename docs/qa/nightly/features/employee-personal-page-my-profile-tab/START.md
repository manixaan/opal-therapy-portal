...
Two candidates, unchanged since last night: `frontend/current/employees.js` (the People register, under More → People) and `frontend/current/profile.js` + `backend/profile-routes.js` (the pre-existing My Profile tab). See STATUS.md for why both are reported and for today's unrelated minor edit in `profile-routes.js`.
