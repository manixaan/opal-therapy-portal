...
| 24 | `OPAL_LETTER_DOCUMENT_ID` | 1 | **footer6** | server | `LTR-` + first 8 of the draft uuid, **issued at draft creation** | `server` | **yes** (always issued) |

**24 unique tags, 26 occurrences.** A per-report `scalarOverrides` entry beats
**every** layer, the Opal-issued document reference included, and the structured
recipient / letter-details fields beat a raw override for the nine tags they own
— those fields are what the wizard writes and what the therapist can see.

### Document control is issued, not looked up

The document reference used to be minted at generate time, which meant the
review step showed it as **Missing** — a field no therapist could possibly go
and find, because the document did not exist yet.

It is now **issued once, when the draft is created**, and stored in
`fca_report_drafts.document_control` alongside the FCA report's. Generate
re-reads that row rather than minting anything, so **regenerating a letter can
never renumber it**. A draft created before this behaviour existed carries `{}`
and resolves from its own id and `created_at`, yielding exactly the reference it
would always have had.

It resolves with source `server`, badges as **"Generated by Opal"**, and remains
**overridable**: a practice that numbers its own correspondence is not overruled
by Opal's convenience. The letter date has always defaulted to today at draft
creation for the same reason, and the FCA report now matches on all four of its
document-control fields.

**Nothing else is auto-filled.** A recipient, a salutation, a reporting period
and a subject are facts about the world; they stay `missing` until a human
supplies them.
...
**An EXCLUDED tag is never outstanding.** The therapist has already answered the
question, the engine writes an empty control rather than a placeholder, and
guard 3 below still checks the finished bytes — so refusing a request they have
already thought about would protect nothing and block real work.
...
## 5a. Excluding a field

A therapist may say outright that a detail does not apply. That is a different
statement from "we could not find this", and produces a different letter.

| State | In the review step | In the .docx |
|---|---|---|
| Has a value | the value + its source badge | the value |
| **Missing** | "No value" + `Missing` badge | the template's own `[PORTAL — …]` placeholder (or, for the six optional lines, the whole line removed) |
| **Excluded** | "Excluded — nothing will be inserted", row de-emphasised, input disabled | **the whole `w:p` removed** for the six optional-line tags; **an empty content control** for every other tag — still there to type into in Word, but blank |

**The template chooses the shape, not the caller.** The engine takes one option,
`manifest.excludedTags`, and folds an excluded optional-line tag into the
existing paragraph-removal pass while rewriting every other excluded tag to an
empty string. Nothing else in the pipeline special-cases exclusion.

- Persisted as `fca_report_drafts.excluded_fields` (JSONB array, migration
  `022`, shared with the FCA report), accepted on `PATCH` as `excludedFields`,
  carried on the manifest as `excludedTags`, **frozen with the snapshot**.
- **Replaced, not merged.** Exclusion is a set the therapist owns outright; a
  merge could not express un-excluding.
- **Excluding clears any override** for that tag, so un-excluding falls back to
  whatever the layers resolve rather than resurrecting a value the therapist
  last saw struck through.
- **An excluded field never blocks generation**, required or not.
- Unknown tags are dropped rather than trusted.

The review step carries one quiet inline note above the merged-values list,
worded identically in both builders:
...
Migration `022` adds two more shared columns, used by both document types:

```sql
ALTER TABLE fca_report_drafts ADD COLUMN document_control JSONB NOT NULL DEFAULT '{}';
ALTER TABLE fca_report_drafts ADD COLUMN excluded_fields  JSONB NOT NULL DEFAULT '[]';
```

`document_control` holds `{ documentReference, reportDate, reportVersion,
reportStatus }`, issued once at draft creation. This template carries a control
only for the reference; the other three are stored for a future template version
rather than being invented at the moment one appears.
...
| Method | Path | Notes |
|---|---|---|
| GET | `/api/letters/template` | blocks, scalar tags, required, optional-line and **excludable** tags, recipient targets |
...
| PATCH | `/api/letters/drafts/:id` | `recipient`, `ccRecipients`, `letterDetails`, `selectedSections`, `customSections`, `scalarOverrides`, `excludedFields` |
| POST | `/api/letters/drafts/:id/save-recipient-to-profile` | `{ target }` — **explicit only** |
| POST | `/api/letters/drafts/:id/generate` | `{ documentId, filename, missingFields, excludedFields, warnings }` |
...
| Suite | Tests |
|---|---|
| `backend/tests/letter-template-map.test.js` | 25 — the sha256 pin and the whole verified contract, discovered from the file |
| `backend/tests/letter-docx-engine.test.js` | 39 — composition and exclusion against the real template |
| `backend/tests/letter-frontend-helpers.test.js` | 94 — pure helpers, exclusion semantics and source guards |
| `backend/tests/integration/progress-note-letters.itest.js` | 55 — real Express, real SQL, real DOCX bytes |
