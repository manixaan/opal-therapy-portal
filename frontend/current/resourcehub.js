...
(function (global) {
  ...
  // Register entries sit at clinician tier; read_only cannot read that tier and
  // the server returns them an empty list, so the tab is hidden for them.
  function canSeeInstruments() { return isOwner() || isAdminRole() || role() === 'therapist'; }
  ...
  /** Can this assessment be opened for a client at all? */
  ...
  function assessSearch(v) {
    ...
    if (String(v || '').trim().length < 2) { S.assess.results = null; render(); return; }
    assessDebounce = setTimeout(async function () {
      S.assess.loading = true; S.assess.err = ''; render();
      // The client search shared by the document builders — org-scoped, live
      // from Splose, never fabricated.
      var d = await api('/api/fca/clients?q=' + encodeURIComponent(S.assess.q));
      S.assess.loading = false;
      if (d.ok) S.assess.results = d.clients || [];
      else { S.assess.results = []; S.assess.err = d.error || 'Client search is unavailable.'; }
      render();
    }, 300);
  }
  ...
  function assessCancel() { S.assess = null; render(); }
