...
(function (global) {
  ...
  function routeFor(view, id) {
    ...
    if (!id) return null;
    return '#assessment/' + v + '/' + encodeURIComponent(String(id));
  }
  ...
  function backLabel() {
    ...
    if (t.kind === 'client') {
      return S.clientName ? ('Back to ' + S.clientName + '’s assessments') : 'Back to this client’s assessments';
    }
  ...
  function renderClientView() {
    ...
    if (!d) {
      host.innerHTML = '<p class="assess-quiet" role="status">Loading…</p>';
      return;
    }
  ...
  function renderHistory() {
    ...
    if (!rows.length) {
      return h + '<p class="assess-empty">No assessments have been recorded for this client yet.</p></section>';
    }
  ...
  function loadRecords() {
    if (!S.clientId) { S.records = []; return Promise.resolve([]); }
    return api('/clients/' + encodeURIComponent(S.clientId) + '/records')
      .then(function (d) { S.records = d.records || []; return S.records; })
      .catch(function () { S.records = []; return []; });
  }
  ...
  function openForClient(key, clientId, clientName) {
    ...
    return Promise.all([loadDefinition(key), loadRecords()])
      .then(function () { S.loading = false; renderView(); })
      .catch(function (err) {
        S.loading = false;
